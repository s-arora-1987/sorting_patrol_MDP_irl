...
class IRLApprximateSolver(object):
    '''Solves the inverse reinforcement learning problem'''
    def __init__(self, max_iter, mdp_solver, n_samples=500, error=.01):
        '''
...
class IteratingPolicyEvaluator2(PolicyEvaluator):
    def __init__(self, error):
    ...
    def evaluate_policy(self, model, agent):
        ...
        V(s) = E[sum_t gamma^t r_t | pi]'''
        V = util.classes.NumMap()
        delta = self._max_error + 1
        while delta > self._max_error:
            (V, delta) = self.iter(model, agent, V)
        return V
